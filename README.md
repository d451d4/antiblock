...
Обход Интернет-цензуры стандартными средствами (SSH-туннелирование)
  ...
  * [Коротко об SSH-туннелировании](#коротко-об-ssh-туннелировании)
  * [Подготовка клиента (локального компьютера)](#подготовка-клиента-локального-компьютера)
    + [Windows: установка OpenSSH клиента](#windows-установка-openssh-клиента)
    + [Windows: генерация RSA-ключей](#windows-генерация-rsa-ключей)
    + [MacOS и Linux: генерация RSA-ключей](#macos-и-linux-генерация-rsa-ключей)
    + [Использование полученного открытого SSH-ключа](#использование-полученного-открытого-ssh-ключа)
  * [Запуск виртуальной машины](#запуск-виртуальной-машины)
    + [Digital Ocean](#digital-ocean)
  * [Подключение по SSH](#подключение-по-ssh)
    + [Подключение к своей виртуальной машине](#подключение-к-своей-виртуальной-машине)
    + [Подключение к чужой виртуальной машине](#подключение-к-чужой-виртуальной-машине)
    + [Предоставление доступа к своей виртуальной машине](#предоставление-доступа-к-своей-виртуальной-машине)
  * [Использование SSH-туннеля в качестве SOCKS прокси](#использование-ssh-туннеля-в-качестве-socks-прокси)
  * [Использование прокси-сервера Squid через SSH-туннель](#использование-прокси-сервера-squid-через-ssh-туннель)
    + [Переадресация порта через SSH-туннель](#переадресация-порта-через-ssh-туннель)
    + [Использование HTTP/HTTPS прокси сервера для обхода блокировок](#Использование-httphttps-прокси-сервера-для-обхода-блокировок)
...
косая черта и точка. Тильда находится слева от клавиши "1" либо слева от клавиши "Я"; косая черта - справа, в предпоследнем ряду клавиш.
...
Чтобы создать и запустить виртуальную машину на DigitalOcean, нужно:
...
5. Выбрать настройки:
  ...
  - $5/mo ($0.007/hour) **ВНИМАНИЕ!** Обязательно удостоверьтесь что выбран самый дешевый пакет!
По умолчанию DigitalOcean предлагает пакет в 40 долларов в месяц!
...
от региона к региону, но предугадать ее трудно).
  - Authentication - выбрать "SSH keys"
  - Нажать кнопку "New SSH Key". В открывшемся окне в поле "SSH key content" вставить
публичную часть ключа [(см. выше)](#подготовка-клиента-локального-компьютера), а
в поле "Name" - имя ключа (любое, на ваше усмотрение). Затем нажать "Add SSH Key"
  - Все остальные настройки оставить без изменения
6. Прокрутить до самого низа страницы и нажать кнопку "Create Droplet"
7. Через несколько минут виртуальная машина будет запущена, и отобразится в списке "Droplets".

Параметры только что созданной виртуальной машины:

* **IP-адрес** (IP Address) виден в списке как четыре числа разделенные точкой. Его можно скопировать.
* **Имя пользователя** (username) - `root`.

Виртуальная машина будет работать непрерывно, и после двухмесячного промо-периода начнет съедать баланс,
5 долларов в месяц. Поэтому если так случилось что ей не пользуетесь, разумно будет ее ликвидировать
нажав в списке "Droplets" опцию "More > Destroy".
...
Поскольку Windows не поддерживает SOCKS-прокси, можно использовать
обычный HTTP/HTTPS прокси. Его необходимо установить на виртуальной
машине, а через SSH-туннель переадресовать порт.

### Установка прокси-сервера Squid

Владелец виртуальной машины должен зайти на нее используя следующую команду
в терминале (командной строке):

    ssh username@ipaddress

В окне отобразится строка приглашения - но уже не локального компьютера,
а виртуальной машины. Из нее можно запускать команды на виртуальной машине.

Чтобы установить прокси-сервер следует запустить команду:

    sudo apt-get install squid -y
    
и дождаться окончания установки пока строка приглашения виртуальной машины
не отобразиться снова. Затем нужно выйти из виртуальной машины нажав
комбинацию клавиш `Ctrl + D` или закрыв окно терминала.

### Переадресация порта через SSH-туннель

В окне терминала (командной строки) запустить следующую команду:

     ssh -Nv -L 3128:localhost:3128
...
### Использование HTTP/HTTPS прокси сервера для обхода блокировок

В Windows нужно зайти в "Параметры > Сеть и Интернет > Прокси-сервер",
включить "Использовать прокси-сервер", и ввести параметры:
Адрес: `localhost`, Порт: `3128`. Подробная инструкция тут:
https://proxys.io/ru/blog/nastroyka-proksi/nastroyka-proksi-servera-windows-10

В MacOS необходимо открыть настройки
сети, выбрать текущее соединение, нажать кнопку "Дополнительно...",
выбрать вкладку "Прокси", пометить оба чекбокса "HTTP Proxy" и "HTTPS Proxy",
и для каждого из них указать "Прокси сервер" `localhost`, а во
втором поле (после двоеточия) - 3128. Затем "OK" и "Применить".
